...
export function analyzeHistorically(option: TerminalOption): AnalystVerdict {
  ...
  const actionAlMap: Record<AnalystVerdict["action"], string> = {
    "BUY CALL": "BLINI CALL — Pritet që çmimi i aksionit të rritet. Kur blini Call, fitoni nëse aksioni ngrihet mbi Strike + Premium deri në skadim.",
    "SELL CALL": "SHITNI CALL (Covered Call) — Pritet që çmimi të qëndrojë i qetë ose të bjerë. Kur shisni Call (kurse i keni aksionet), mbani premiumin edhe nëse aksioni nuk lëviz.",
    "BUY PUT": "BLINI PUT — Pritet që çmimi i aksionit të bjerë. Kur blini Put, fitoni nëse aksioni bie nën Strike - Premium deri në skadim.",
    "SELL PUT": "SHITNI PUT (Cash-Secured Put) — Pritet që çmimi të qëndrojë i qetë ose të rritet. Kur shisni Put, mbani premiumin nëse aksioni qëndron mbi Strike. Nëse bie nën Strike, duhet të blini aksionin me çmim Strike.",
    ...
    "AVOID": "SHMANGNI — Cilësia e të dhënave është shumë e ulët ose rreziku është i lartë. Mos hyni në tregti me këtë aksion derisa kushtet të përmirësohen."
  };
  ...
  const whyAlMap: Record<AnalystVerdict["action"], string> = {
    ...
    "SELL CALL": `IV është e lartë (${pct(option.iv)}) krahasuar me HV (${pct(historical.realizedVol30)}), çka do të thotë se opsionet janë të shtrenjta. Si shitës i Call, përfitoni nga premiumi i lartë dhe nga rënia e IV (IV crush). Nëse aksioni nuk lëviz shumë, mbani gjithë premiumin.`,
    ...
    "SELL PUT": `IV është e lartë (${pct(option.iv)}) krahasuar me HV (${pct(historical.realizedVol30)}). Si shitës i Put, mbani premiumin nëse aksioni qëndron mbi Strike. Nëse bie nën Strike, e blini aksionin me çmim të zvogëluar (me zbritje nga premiumi). Kjo është strategji që përdoret kur jeni neutral-bullish.`,
  ...
  const howAlMap: Record<AnalystVerdict["action"], string> = {
    "BUY CALL": `1. Zgjidhni Call me Strike afër ATM (Delta 0.45-0.60) dhe DTE 30-45.\n2. Hyni me madhësi pozicioni ${macro.positionScale} (ose më të vogël).\n3. Vendosni Stop Loss nëse çmimi bie 15-20% nga premium.\n4. Merreni fitimin nëse opsioni bëhet 2x premiumi.\n5. Mbyllni para skadimit javën e fundit (rrezik gamma).`,
    "SELL CALL": `1. Sigurohuni që keni 100 aksione për covered call (ose përdorni spread).\n2. Zgjidhni Strike OTM 5-10% mbi çmimin aktual.\n3. DTE 30-45 për ekuilibër midis premium dhe rrezikut.\n4. Mbyllni nëse aksioni ngrihet afër Strike.\n5. Mos e shitni Call pa mbulim (naked) — rrezik i pakufizuar.`,
    "BUY PUT": `1. Zgjidhni Put me Strike afër ATM (Delta -0.45 deri -0.60) dhe DTE 30-45.\n2. Hyni me madhësi ${macro.positionScale}.\n3. Vendosni Stop Loss nëse çmimi nuk bie brenda 7-10 ditëve.\n4. Merreni fitimin nëse opsioni bëhet 2x premiumi.\n5. Përdorni Put Debit Spread për të ulur rrezikun.`,
    "SELL PUT": `1. Sigurohuni që keni kapital për 100 aksione (cash-secured).\n2. Zgjidhni Strike OTM 5-10% nën çmimin aktual.\n3. DTE 30-45.\n4. Nëse aksioni bie nën Strike, blini me zbritje.\n5. Nëse aksioni qëndron mbi Strike, mbani premiumin (fitim 100%).\n6. Përdorni Put Credit Spread për rrezik të përcaktuar.`,
    "WATCH": `1. Shtojeni ${option.symbol} në listën e vëzhgimit.\n2. Kontrolloni çdo ditë ndryshimin e Score dhe IV.\n3. Hyni vetëm kur Score > 65 dhe IV < HV.\n4. Kontrolloni datën e earnings para se të hyni.\n5. Mos u nënshtrohuni FOMO-së — tregti më të mira vijnë me durim.`,
    "AVOID": `1. Mos hyni në asnjë pozicion me këtë aksion tani.\n2. Prisni derisa Data Quality të kalojë 60+.\n3. Kontrolloni përsëri pas ditës tjetër të tregtimit.\n4. Nëse duhet ekspozim në sektor, kërkoni aksion me Score më të lartë.\n5. Konsideroni aksion të ngjashëm me cilësi më të mirë të dhënash.`
  };
  ...
  const riskAlMap: Record<AnalystVerdict["riskLabel"], string> = {
    ...
    "Medium": `Rrezik mesatar — IV është relativisht e lartë (${pct(option.iv)}) ose maksimumi i rënies është ${pct(historical.maxDrawdown)}. Përdorni pozicione më të vogla, spreade për rrezik të përcaktuar, dhe kontrolloni datën e earnings.`,
    "High": `Rrezik i lartë — IV është shumë e lartë (${pct(option.iv)}), rreziku makro është ${macro.riskScore}/100, ose jeni duke shitur opsione pa mbulim. PËRDORNI VETËM SPREADE ME RREZIK TË PËRCAKTUAR. Mos i shitni pa mbulim.`,
    "Very High": `RREZIK SHUMË I LARTË — cilësia e të dhënave është ${option.dataQuality}/100, çka do të thotë se të gjitha metrikat janë të pasakta ose sintetike. MOS TREGTONI me këtë aksion me para reale. Përdorni vetëm për demonstrim.`
  };
